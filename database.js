...
module.exports = {
  ...
  createJob:  (name, address, cb) =>{

    // geocode the address using google maps api


    // td: geocode the address and store the lat and long

    con.query('INSERT INTO jobs (name, address) VALUES (?, ?);', [name, address], (err) => {
      if (!err){
        cb(err);
      } else {
...
//  PRIMARY KEY (id)
    ...
    con.query('SELECT timesheet.user_id AS UserID, users.name as name, SUM(timesheet.duration) AS TotalDuration FROM timesheet JOIN users ON users.id = timesheet.user_id GROUP BY timesheet.user_id;', (err, rows) =>{
  ...
  getUserHours: (userid, cb) => {
    ...
    con.query('SELECT timesheet.user_id AS UserID, users.name AS name, timesheet.duration, timesheet.clock_in,  TIMEDIFF(clock_out, clock_in) as duration, notes, job_id FROM timesheet JOIN users ON users.id = timesheet.user_id WHERE timesheet.user_id = ? ORDER BY timesheet.clock_in DESC;', [userid], (err, rows) =>{
      if (!err && rows !== undefined && rows.length > 0) {
        rows.forEach((row) => {
  ...
  clockIn: (id, cb) => {
    if (id){
      con.query('INSERT INTO timesheet (user_id, clock_in) values (?, NOW()); SELECT * FROM timesheet WHERE id = LAST_INSERT_ID(); UPDATE users SET clockedIn = 1 where id = ?;', [id, id], (err, rows) =>{
          if (!err) {
            cb(err);
          } else {
  ...
  getClockInDuration: (userId, cb) => {
    if (userId != null){
      con.query('SELECT * FROM timesheet WHERE user_id = ? AND clock_out is NULL;', [userId], (err,rows) => {
  ...
  updateLocation: (userId, location, cb) => {
    con.query('UPDATE users SET recent_location = ? WHERE id = ?;', [location, userId], (err) => {
      if (!err){
        con.query('UPDATE users SET last_seen = NOW() WHERE id = ?;', [userId], (err) => {
          if (!err){
            cb(err);
          } else {
            cb(err || "Failed to update last seen");
          }
        });
      } else {
        cb(err || "Failed to update location");
      }
    });
  },
    ...
    con.query('select * from timesheet where user_id = ? and clock_out is NULL;', [userId], (err, rows) => {
      if (!err && rows !== undefined && rows[0]){
        con.query('UPDATE timesheet SET clock_out = NOW(), notes = ? WHERE user_id = ? AND clock_out IS NULL; UPDATE users SET clockedIn = 0 WHERE id = ?;', [notes, userId, userId], (err) =>{
          if (!err) {
...
// need to be updated
  clockInAndOut: (userId, jobId, cb) => {
    con.query('SELECT clockedIn FROM users WHERE (id = ?);', [userId], (err, rows) =>{
      if (!err && rows !== undefined && rows.length > 0) {
      
        if (rows[0].clockedIn == 0){
          //clockin
          con.query('INSERT INTO timesheet (user_id, job, clock_in) values (?, ?, ?, NOW()); SELECT * FROM timesheet WHERE id = LAST_INSERT_ID(); UPDATE users SET clockedIn = 1 where id = ?;', [userId, jobId, userId], (err, rows) =>{
            if (!err) {
              cb(err);
            } else {
              cb (err || "Failed to clock in.");
          ...
          // clockout 
          con.query('UPDATE timesheet SET clock_out = NOW() WHERE user_id = ? AND clock_out IS NULL; UPDATE users SET clockedIn = 0 WHERE id = ?;', [userId, userId], (err) =>{
            if (!err) {
              cb(err);
            } else {
              cb (err || "Failed to clock out.")
  ...
  clockOutAll: (cb) => {
    ...
    getWholeTimesheet: (req, res, cb) => {
      con.query('SELECT timesheet.id as uid, timesheet.user_id, timesheet.job_id, timesheet.clock_in, timesheet.clock_out, timesheet.duration, timesheet.notes, jobs.name, jobs.isArchived, users.id, users.name AS username FROM timesheet LEFT JOIN jobs ON timesheet.job_id = jobs.id AND jobs.isArchived = 0 AND timesheet.clock_out IS NOT NULL INNER JOIN users ON timesheet.user_id = users.id ORDER BY timesheet.clock_in ASC;', (err, rows) => {
        if (!err) {
          cb(rows);
        } else {
  ...
  getTimesheet: (req, res, cb) => {
    con.query('SELECT timesheet.id as uid, timesheet.user_id, timesheet.job_id, timesheet.clock_in, timesheet.clock_out, timesheet.duration, jobs.name, jobs.isArchived, users.id, users.name AS username FROM timesheet JOIN jobs ON timesheet.job_id = jobs.id AND jobs.isArchived = 0 AND timesheet.clock_out IS NOT NULL INNER JOIN users ON timesheet.user_id = users.id ORDER BY timesheet.clock_in ASC;', (err, rows) => {
      if (err) {
        console.log(err);
        return cb(err);
      }
  ...
  getTimesheetQuery: (req, res, startDate, endDate, userId, jobId, weekId,  cb) => {
  ...
  con.query('SELECT timesheet.id as uid, timesheet.user_id, timesheet.job_id, timesheet.task, timesheet.clock_in, timesheet.clock_out, TIMEDIFF(timesheet.clock_out, timesheet.clock_in) as duration, timesheet.notes, jobs.name, jobs.isArchived, users.id, users.name AS username, tasks.name AS taskname FROM timesheet LEFT JOIN jobs  ON  timesheet.job_id = jobs.id AND jobs.isArchived = 0 AND timesheet.clock_out IS NOT NULL INNER JOIN users ON timesheet.user_id = users.id LEFT JOIN tasks ON tasks.id = timesheet.task AND tasks.isArchived = 0 ORDER BY timesheet.clock_in;', (err, rows) => {
  ...
  //        if (rows[i].duration == undefined){
      ...
      var duplicates = false;
        ...
        for (var i = 0; i < rows.length; i++){
          for (var j = i; j < rows.length; j++){
            duplicates = false;
            if (i != j && rows[i].isArchived ==0 && rows[j].isArchived == 0 && rows[i].user_id == rows[j].user_id && rows[i].job == rows[j].job){
              duplicates = true;
    ...
    getTestData: function(n){
      ...
      for (var i = 0; i < n; i++){
        ...
        con.query('INSERT INTO timesheet (user_id, job, task, clock_in, clock_out) VALUES (1, 1, 1, ?, ?)', [randomDate.format('YYYY-MM-DD'), randomDate.format('YYYY-MM-DD')]);
